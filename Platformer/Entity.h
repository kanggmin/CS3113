...
enum EntityType { PLATFORM, PLAYER, ENEMY, PORTAL };
enum AIType { WALKER };
...
public:
	...
	Entity(EntityType type, glm::vec3 pos, float speed, glm::vec3 accel, GLuint sprite,
		int* left_anim, int* right_anim, int frames, int index, float time, int cols, int rows, float heigh, float width);
	~Entity();
